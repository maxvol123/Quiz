...
import { Htmlquestions } from './components/questions';
...
function App() {
  ...
  const [point, setPoint]=useState(0)
    ...
    if (startgame===true) {
      if (curentquestion + 1 > Htmlquestions.length) {
        setStartgame(false)
        setFinish(true)
        setCurentquestion(0)
      }  
  ...
  return (
    <div className="App">
      ...
      {game&&game_elements.map(product=><Game StartG={()=>{setStartgame(true); setGame(false)}} element={product} key={product.id}/>)}
      {startgame&&<Start point={point} setPoint={setPoint} AddPoint={()=>setCurentquestion(curentquestion+1)} curentquestion={curentquestion} element={Htmlquestions}/>}
